...
- A local unsigned `0.7.0.1` x64 development MSIX was generated before publication. Its `HelloThisWorld.winTerm` identity, `CN=winTerm Development` publisher, `winterm.exe` alias, absence of `wt.exe`, and terminal-host product metadata passed package-output inspection.
