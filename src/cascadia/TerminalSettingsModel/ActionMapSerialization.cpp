    ...
    std::vector<SettingsLoadWarnings> ActionMap::LayerJson(const Json::Value& json, const OriginTag origin, const bool withKeybindings)
    {
        ...
        for (const auto& jsonBlock : json)
        {
            ...
            else if (keys)
            {
                ...
                // winTerm reserves Ctrl+C for interrupting the foreground process.
                // Older releases wrote this copy binding to user settings, so remove
                // it during load and persist the migrated settings.
                const auto keyJson{ jsonBlock.find(&*KeysKey.cbegin(), (&*KeysKey.cbegin()) + KeysKey.size()) };
                if (origin == OriginTag::User &&
                    idJson == L"Terminal.CopyToClipboard" &&
                    keyJson->asString() == "ctrl+c")
                {
                    _fixupsAppliedDuringLoad = true;
                    continue;
                }
                ...
                if (!_changeLog.contains(KeysKey.data()))
                {
                    ...
                    static constexpr std::array<std::pair<std::wstring_view, std::string_view>, 2> userDefaultKbds{ { { L"Terminal.PasteFromClipboard", "ctrl+v" },
