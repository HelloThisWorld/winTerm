...
try
{
    ...
    Assert-Condition ($version.applicationVersion -eq '0.7.0-beta.3') 'Application version is 0.7.0-beta.3'
    Assert-Condition ($version.packageVersion -eq '1.0.5.0') 'Package version is 1.0.5.0'
    Assert-Condition ($version.moduleVersion -eq '0.7.0') 'PowerShell module version is 0.7.0'
    Assert-Condition ($version.modulePrerelease -eq 'beta.3') 'PowerShell module prerelease is beta.3'
    Assert-Condition ($version.channel -eq 'beta') 'Release channel is beta'
    Assert-Condition ($version.tag -eq 'v0.7.0-beta.3') 'Beta tag is v0.7.0-beta.3'
    ...
    Assert-Condition ((Get-Text 'shell\powershell\winTerm.Shell\winTerm.Shell.psm1').Contains("`$script:WinTermModuleVersion = '0.7.0-beta.3'")) 'PowerShell module runtime version matches release metadata'
    ...
    Assert-Condition ($releaseHeader.Contains('ApplicationVersion{ L"0.7.0-beta.3" }')) 'About metadata application version is 0.7.0-beta.3'
    ...
    Assert-Condition ((Get-Text 'src\winterm\Workspaces\Persistence\WorkspaceSerializer.cpp').Contains('"0.7.0-beta.3"')) 'Workspace application-version fallback is 0.7.0-beta.3'
    ...
    Assert-Condition ((Get-Text 'CHANGELOG.md').Contains('## 0.7.0-beta.3')) 'Changelog contains 0.7.0-beta.3'

    if ($RequireTag)
    {
        $tag = (& git describe --tags --exact-match 2>$null).Trim()
        Assert-Condition ($LASTEXITCODE -eq 0 -and $tag -eq $version.tag) 'Checked-out commit is exactly tagged v0.7.0-beta.3'
    }
