...
try
{
    ...
    Assert-Condition ($version.applicationVersion -eq '1.3.0-beta2') 'Application version is 1.3.0-beta2'
    Assert-Condition ($version.packageVersion -eq '1.3.0.5') 'Package version is 1.3.0.5'
    Assert-Condition ($version.moduleVersion -eq '1.3.0') 'PowerShell module version is 1.3.0'
    Assert-Condition ($version.modulePrerelease -eq 'beta2') 'PowerShell module prerelease suffix is beta2'
    ...
    Assert-Condition ($version.tag -eq 'v1.3.0-beta2') 'Release tag is v1.3.0-beta2'
    ...
    Assert-Condition ($releaseHeader.Contains('ApplicationVersion{ L"1.3.0-beta2" }')) 'About metadata application version is 1.3.0-beta2'
    ...
    Assert-Condition ((Get-Text 'src\winterm\Workspaces\Model\WorkspaceDescriptor.h').Contains('applicationVersion{ "1.3.0-beta2" }')) 'Workspace model application-version fallback is 1.3.0-beta2'
    ...
    Assert-Condition ((Get-Text 'src\winterm\Workspaces\Persistence\WorkspaceSerializer.cpp').Contains('"1.3.0-beta2"')) 'Workspace serializer application-version fallback is 1.3.0-beta2'
    ...
    if ($RequireTag)
    {
        ...
        Assert-Condition ($LASTEXITCODE -eq 0 -and $tag -eq $version.tag) 'Checked-out commit is exactly tagged v1.3.0-beta2'
    }
