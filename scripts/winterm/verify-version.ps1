...
try
{
    ...
    Assert-Condition ($version.applicationVersion -eq '1.2.0') 'Application version is 1.2.0'
    Assert-Condition ($version.packageVersion -eq '1.2.0.0') 'Package version is 1.2.0.0'
    Assert-Condition ($version.moduleVersion -eq '1.2.0') 'PowerShell module version is 1.2.0'
    ...
    Assert-Condition ($version.tag -eq 'v1.2.0') 'Stable tag is v1.2.0'
    ...
    Assert-Condition ((Get-Text 'shell\powershell\winTerm.Shell\winTerm.Shell.psm1').Contains("`$script:WinTermModuleVersion = '1.2.0'")) 'PowerShell module runtime version matches release metadata'
    ...
    Assert-Condition ($releaseHeader.Contains('ApplicationVersion{ L"1.2.0" }')) 'About metadata application version is 1.2.0'
    ...
    Assert-Condition ((Get-Text 'src\winterm\Workspaces\Model\WorkspaceDescriptor.h').Contains('applicationVersion{ "1.2.0" }')) 'Workspace model application-version fallback is 1.2.0'
    ...
    Assert-Condition ((Get-Text 'src\winterm\Workspaces\Persistence\WorkspaceSerializer.cpp').Contains('"1.2.0"')) 'Workspace serializer application-version fallback is 1.2.0'
    ...
    Assert-Condition ((Get-Text 'CHANGELOG.md').Contains('## 1.2.0 - 2026-08-01')) 'Changelog contains 1.2.0'

    if ($RequireTag)
    {
        $tag = (& git describe --tags --exact-match 2>$null).Trim()
        Assert-Condition ($LASTEXITCODE -eq 0 -and $tag -eq $version.tag) 'Checked-out commit is exactly tagged v1.2.0'
    }
