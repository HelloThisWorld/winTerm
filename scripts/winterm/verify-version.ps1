...
try
{
    ...
    Assert-Condition ($version.applicationVersion -eq '1.0.0') 'Application version is 1.0.0'
    Assert-Condition ($version.packageVersion -eq '1.0.0.0') 'Package version is 1.0.0.0'
    Assert-Condition ($version.moduleVersion -eq '1.0.0') 'PowerShell module version is 1.0.0'
    Assert-Condition ($version.channel -eq 'stable') 'Release channel is stable'
    Assert-Condition ($version.tag -eq 'v1.0.0') 'Release tag is v1.0.0'
    ...
    Assert-Condition ((Get-Text 'shell\powershell\winTerm.Shell\winTerm.Shell.psm1').Contains("`$script:WinTermModuleVersion = '1.0.0'")) 'PowerShell module runtime version matches release metadata'
    ...
    Assert-Condition ($releaseHeader.Contains('ApplicationVersion{ L"1.0.0" }')) 'About metadata application version is 1.0.0'
    ...
    Assert-Condition ((Get-Text 'src\winterm\Workspaces\Persistence\WorkspaceSerializer.cpp').Contains('"1.0.0"')) 'Workspace application-version fallback is 1.0.0'

    $releaseWorkflow = Get-Text '.github\workflows\release.yml'
    Assert-Condition ($releaseWorkflow.Contains('v1.0.0')) 'Release workflow targets v1.0.0'
    Assert-Condition ($releaseWorkflow.Contains('winTerm 1.0.0')) 'Release workflow title is winTerm 1.0.0'
    Assert-Condition ($releaseWorkflow.Contains('winTerm-1.0.0-x64.msix')) 'Release workflow uses the required x64 artifact name'

    $releaseNotes = Get-Text 'docs\releases\1.0.0.md'
    Assert-Condition ($releaseNotes.Contains('# winTerm 1.0.0')) 'Release notes title is winTerm 1.0.0'
    Assert-Condition ((Get-Text 'CHANGELOG.md').Contains('## 1.0.0')) 'Changelog contains 1.0.0'

    if ($RequireTag)
    {
        $tag = (& git describe --tags --exact-match 2>$null).Trim()
        Assert-Condition ($LASTEXITCODE -eq 0 -and $tag -eq $version.tag) 'Checked-out commit is exactly tagged v1.0.0'
    }
