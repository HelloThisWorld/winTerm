...
try
{
    ...
    Assert-Condition ($version.applicationVersion -eq '1.1.2') 'Application version is 1.1.2'
    Assert-Condition ($version.packageVersion -eq '1.1.2.0') 'Package version is 1.1.2.0'
    Assert-Condition ($version.moduleVersion -eq '1.1.2') 'PowerShell module version is 1.1.2'
    ...
    Assert-Condition ($version.tag -eq 'v1.1.2') 'Stable tag is v1.1.2'
    ...
    Assert-Condition ("$major.$minor" -eq '1.1') 'Executable metadata major and minor versions match 1.1'
    ...
    Assert-Condition ((Get-Text 'shell\powershell\winTerm.Shell\winTerm.Shell.psm1').Contains("`$script:WinTermModuleVersion = '1.1.2'")) 'PowerShell module runtime version matches release metadata'
    ...
    Assert-Condition ($releaseHeader.Contains('ApplicationVersion{ L"1.1.2" }')) 'About metadata application version is 1.1.2'
    ...
    Assert-Condition ((Get-Text 'src\winterm\Workspaces\Persistence\WorkspaceSerializer.cpp').Contains('"1.1.2"')) 'Workspace application-version fallback is 1.1.2'
    ...
    Assert-Condition ((Get-Text 'CHANGELOG.md').Contains('## 1.1.2 - 2026-07-24')) 'Changelog contains 1.1.2'

    if ($RequireTag)
    {
        $tag = (& git describe --tags --exact-match 2>$null).Trim()
        Assert-Condition ($LASTEXITCODE -eq 0 -and $tag -eq $version.tag) 'Checked-out commit is exactly tagged v1.1.2'
    }
